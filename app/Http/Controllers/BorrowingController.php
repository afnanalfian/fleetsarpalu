...
class BorrowingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($user->hasRole(['admin', 'kepala sumber daya'])) {
            // admin/sumdas → lihat semua
        } else {
            // Pegawai / Ketua Tim / External → hanya:
            // 1. peminjam sendiri
    ...
    public function store(Request $request)
    {
        ...
        if ($user->role === 'External') {
            $rules['lampiran'] = 'nullable|file|mimes:pdf|max:2048';
        } else {
            $rules['surat_tugas'] = 'nullable|file|mimes:pdf|max:2048';
        }
        ...
        if ($request->hasFile('surat_tugas')) {
            $fileSuratTugas = $request->file('surat_tugas')->store('surat_tugas', 'public');
        }

        if ($request->hasFile('lampiran')) {
            $fileLampiran = $request->file('lampiran')->store('lampiran', 'public');
        }

        // Generate kode pinjam otomatis: BR-YYYYMMDD-XXX
        $prefix = $user->role === 'External' ? 'XBR' : 'BR';
        $today = now()->format('Ymd');
        $countToday = BorrowRequest::whereDate('created_at', now())->count() + 1;
        $sequence = str_pad($countToday, 3, '0', STR_PAD_LEFT);

        $kodePinjam = "{$prefix}-{$today}-{$sequence}";
        ...
        $borrow = BorrowRequest::create([
            ...
            'surat_tugas_path'    => $user->role === 'External' ? null : $fileSuratTugas,
            'lampiran_path'       => $user->role === 'External' ? $fileLampiran : null,
    ...
    public function edit($id)
    {
        ...
        $vehicles = Vehicle::where('status', 'available')
            ...
            ->orderBy('name')
            ->get();
        $isExternal = $borrow->user->isExternal();

        return view('borrowings.edit', compact('borrow', 'vehicles', 'isExternal'));
    }
    public function update(Request $request, $id)
    {
        ...
        if ($isExternal) {
            if ($request->hasFile('lampiran')) {
                $data['lampiran_path'] = $request->file('lampiran')->store('lampiran', 'public');
            }
        } else {
            if ($request->hasFile('surat_tugas')) {
    ...
    public function assignDriver(Request $request, $id)
    {
        ...
        // Hanya Kepala Sumber Daya yang boleh
        if (auth()->user()->role !== 'Kepala Sumber Daya') {
            abort(403);
        }
        ...
        notify(
            $driver->id,
            "Anda Ditugaskan Sebagai Supir",
            "Anda ditugaskan untuk peminjaman kendaraan {$borrow->kode_pinjam}. Silakan cek detail peminjaman.",
            route('borrowings.show', $borrow->id)
        );

        return back()->with('success', 'Supir berhasil dipilih dan notifikasi dikirim.');
    }
